...
public class InventoryUtils
{
    ...
    public static void debugPrintSlotInfo(GuiContainer gui, Slot slot)
    {
        ...
        String stackStr = ItemWrap.getStackString(slot.getStack());
    ...
    private static boolean isValidSlot(Slot slot, GuiContainer gui, boolean requireItems)
    {
        return gui.inventorySlots != null && gui.inventorySlots.inventorySlots != null &&
                ...
                Configs.Lists.SLOT_BLACKLIST.getValue().contains(slot.getClass().getName()) == false;
    }
    ...
    public static boolean canShiftPlaceItems(GuiContainer gui)
    {
        ...
        ItemStack stackCursor = GameWrap.getClientPlayer().inventory.getItemStack();
    ...
    public static boolean tryMoveItems(GuiContainer gui, RecipeStorage recipes, boolean scrollingUp)
    {
        ...
        if (slot == null || isStackEmpty(GameWrap.getClientPlayer().inventory.getItemStack()) == false)
        {
            return false;
        }
    ...
    private static boolean dragMoveFromSlotAtPosition(GuiContainer gui, int x, int y, MoveAction action)
    {
        ...
        boolean flag = slot != null && isValidSlot(slot, gui, true) && slot.canTakeStack(GameWrap.getClientPlayer());
    ...
    {
        ItemStack stackReference = GameWrap.getClientPlayer().inventory.getItemStack();
    ...
    private static boolean tryMoveSingleItemToOtherInventory(Slot slot, GuiContainer gui)
    {
        ...
        if (isStackEmpty(player.inventory.getItemStack()) == false || slot.canTakeStack(player) == false ||
        ...
        slot.putStack(stack);
        container.transferStackInSlot(player, slot.slotNumber);
    ...
    private static boolean tryMoveSingleItemToThisInventory(Slot slot, GuiContainer gui)
    {
        ...
        for (int slotNum = container.inventorySlots.size() - 1; slotNum >= 0; slotNum--)
        {
            ...
            if (areSlotsInSameInventory(slotTmp, slot) == false &&
                isStackEmpty(stackTmp) == false && slotTmp.canTakeStack(GameWrap.getClientPlayer()) &&
    ...
    {
        MerchantRecipeList list = gui.getMerchant().getRecipes(GameWrap.getClientPlayer());
    ...
    private static void fillBuySlot(GuiContainer gui, int slotNum, ItemStack buyStack, boolean fillStacks)
    {
        ...
        {
            moveItemsFromInventory(gui, slotNum, GameWrap.getClientPlayer().inventory, buyStack, fillStacks);
        }
    }
            ...
            boolean isLeftClick, boolean isRightClick, boolean isPickBlock, boolean isShiftDown)
    {
        if (isLeftClick || isRightClick)
        {
            ...
            if (isRightClick)
            {
                ...
                boolean dropKeyDown = Keys.isKeyDown(mc.gameSettings.keyBindDrop.getKeyCode());
    ...
    {
        Slot slot = InventoryScreenUtils.getSlotUnderMouse(gui);
        InventoryPlayer inv = GameWrap.getClientPlayer().inventory;
    ...
    {
        ItemStack stackInCursor = GameWrap.getClientPlayer().inventory.getItemStack();
    ...
    private static void moveOneRecipeItemIntoCraftingGrid(GuiContainer gui, Slot slotGridFirst, ItemStack ingredientReference, List<Integer> targetSlots)
    {
        ...
        while (index < slotCount)
        {
            ...
            // Successfully picked up ingredient items
            if (areStacksEqual(ingredientReference, player.inventory.getItemStack()))
            {
    ...
    */
    ...
    private static boolean clickSlotsToMoveSingleItem(GuiContainer gui, int slotFrom, int slotTo)
    {
        ...
        if (isStackEmpty(GameWrap.getClientPlayer().inventory.getItemStack()) == false)
        {
    ...
    {
        InventoryPlayer inventory = GameWrap.getClientPlayer().inventory;
        ...
        if (isStackEmpty(inventory.getItemStack()))
        {
            return false;
        }

        boolean ret = true;
        int size = getStackSize(inventory.getItemStack());
        ...
        if (isStackEmpty(inventory.getItemStack()) == false)
        {
            ret = getStackSize(inventory.getItemStack()) != size;
    ...
    */
    ...
    public static void clickSlot(GuiContainer gui, int slotNum, int mouseButton, ClickType type)
    {
        ...
        else
        {
            try
            {
                malilib.util.inventory.InventoryUtils.clickSlot(gui.inventorySlots, slotNum, mouseButton, type);
            }
            catch (Exception e)
            {
